...
//COM defines
#define serial1_com_baud 2e6
#define serial1_wait_time_secure_faktor 3   //how many time/byte you want to wait for the next byte to arrive, before the algorithm goes on with the next step
#define serial1_max_com_response_time 2     //[ms] how long the protocol should wait, until a data stream begins (max is 32ms anything else is infinite. If you need more change self_max_com_response_time to unsigned long)
...
struct REQUEST_BYTES_SERIAL{ //requierd, so the Slave knows it's an requestbyte and to check the information byte
  byte ping = 0x23;
}request_bytes_serial1;

byte Serial1_read_box() {
  return (Serial1.read());
}
byte Serial1_peek_box() {
  return (Serial1.peek());
}
int  Serial1_available_box() {
  return (Serial1.available());
}
size_t Serial1_write_box(byte b) {
...
COM_LIB_FUN Serial1_lib {Serial1_read_box, Serial1_peek_box, Serial1_available_box, Serial1_write_box};

bool serial1_handle_functions_callback_list(byte r){
 ...
 * -If you recive data and send data in one function:
 *    Save your rx data in the serial_rx_buffer array and use it from their,
 ...
 *    +Make sure the first byte you send back is not an variable byte, only constante byte. Otherwise it could get an error_byte and triger an error.
...
*/
 ...
 switch(r){             //call by request byte
  case 0x23:  get_ping();   break; //call your function as a case
  }
...
Serial_protocol SP1(Serial1_lib, &request_bytes_serial1, sizeof(request_bytes_serial1), serial1_com_baud, serial1_handle_functions_callback_list, serial1_wait_time_secure_faktor, serial1_max_com_response_time);
...
bool get_info(){
  ...
  if(Serial1.read() != 0x22){ //check the fix value
    return(0);
  }

  if(Serial1.available() == sizeof(float)+1) { //check if the amount of bytes is right, +1 because of acknowledge_byte send by the algorithm

  SP1.get_value(&infos);
  
  SP1.flush_serial_buffer(); //making sure the buffer is empty
...
void setup() {
  ...
  Serial.begin(2e6);
  Serial1.begin(serial1_com_baud);
  while (!Serial) {
...
void loop() {
  ...
  static unsigned long timer, timersum = 0, timer_a;
  
  Serial.println("Start Test\n");

  do{
    timer = micros();
    SP1.send_request(0x22, infos, get_info);
    timersum += micros() - timer;
    if(last_Serial_trans_good == 0) g_bad_tran++;
    SP1.call_slave_protocol();
    delayMicroseconds(30);
  }while(infos < 1000000);

  timer_a = ((float)timersum)/(infos + g_bad_tran);

  
  Serial.print("Tests                : ");
  Serial.println(infos + g_bad_tran);
  Serial.print("Pings                : ");
  Serial.println(ping_c);
  Serial.println("");
  Serial.print("Microsesonds/Com     : ");
  Serial.println(timer_a);
  Serial.print("Failed Com           : ");
  Serial.println(g_bad_tran);
  Serial.print("Bad Bytes            : ");
  Serial.println(g_error_num);
  Serial.print("Double Master Problem: ");
  Serial.println(d_master);
